...
import { Mail, Lock, ArrowRight, Timer, User, ChevronDown, ArrowLeft, GraduationCap, BookOpen, Stethoscope, Loader2 } from "lucide-react";
import Link from "next/link";
import { useRouter } from "next/navigation";
import { useAuth } from "@/hooks/useAuth"; // ফায়ারবেস অথ হুক ইমপোর্ট করা হলো

export default function AuthPage() {
  const router = useRouter();
  
  // ফায়ারবেস অথেন্টিকেশন স্টেট এবং ফাংশন
  ...
  // ইউজার আগে থেকেই লগড ইন থাকলে সরাসরি হোমপেজে পাঠিয়ে দেবে
  useEffect(() => {
    if (user && !loading) {
      router.push("/");
    }
  ...
  // ইমেইল/পাসওয়ার্ড ফর্ম সাবমিট লজিক (ভবিষ্যতের জন্য)
  const handleAuth = (e: React.FormEvent) => {
    e.preventDefault();
    if (isSignIn) {
      console.log("Signing in with:", { email, password });
      // ফায়ারবেস ইমেইল লগইন লজিক এখানে বসবে
    } else {
      if (signUpStep === 1) {
        setSignUpStep(2); // প্রথম ধাপ শেষ হলে দ্বিতীয় ধাপে যাবে
      } else {
        console.log("Final Profile Data:", { name, gender, email, password, studyLevel });
        // ফায়ারবেস ইমেইল সাইন-আপ ও প্রোফাইল সেভ করার লজিক এখানে বসবে
      }
    }
  };
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // সফল হলে useEffect অটোমেটিক হোমপেজে রিডাইরেক্ট করবে
    } catch (error) {
  ...
  // ইনিশিয়াল লোডিং স্টেটে ব্ল্যাংক স্ক্রিন না দেখিয়ে পুরো পেজটাকেই লোডিং অবস্থায় রাখা হলো
  if (loading) {
    return (
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-5 bg-[#0f172a] text-white transition-colors duration-300">
      
      {/* Top Logo & Dynamic Welcome Text */}
      <div className="flex flex-col items-center mb-8 text-center mt-10 animate-in fade-in slide-in-from-top-4 duration-500">
        <div className="w-16 h-16 bg-indigo-600 rounded-2xl flex items-center justify-center mb-5 shadow-lg shadow-indigo-500/20 rotate-3 hover:rotate-0 transition-transform duration-300">
      ...
      {/* Main Auth Card */}
      <div className="w-full max-w-sm bg-[#1e293b] rounded-3xl p-6 shadow-xl border border-slate-700/50 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="animate-in fade-in duration-500">
            ...
            <div className="space-y-3">
              <button 
                ...
                className="w-full flex items-center justify-center gap-3 bg-transparent border border-slate-700 hover:bg-slate-800 text-white font-semibold py-3 rounded-xl transition-colors active:scale-95 disabled:opacity-50 disabled:pointer-events-none"
              >
                {isGoogleLoading ? (
